    ...
    <div className="space-y-8">
      ...
      {showUpgradeModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-3xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-300">
            {/* Top banner */}
            ...
            <div className="px-6 pb-6 space-y-3">
              ...
              <Link
                ...
                onClick={() => setShowUpgradeModal(false)}
                className="w-full bg-gradient-to-r from-orange-500 to-orange-600 text-white py-3.5 rounded-2xl text-sm font-semibold uppercase tracking-widest hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-2"
              >
      ...
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl border border-slate-200/60">
        <div>
          <h1 className="text-xl font-semibold text-[#1e3a5f] uppercase tracking-tight">{t("Billing & Finance", "బిల్లింగ్ మరియు ఫైనాన్స్")}</h1>
          <p className="text-[10px] text-slate-400 uppercase tracking-widest font-medium">{t("Financial Overview", "ఆర్థిక సమాచారం")}</p>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={handleGSTClick}
            className="relative bg-slate-50 text-[#1e3a5f] border border-slate-200 px-5 py-2.5 rounded-lg text-xs font-semibold uppercase flex items-center gap-2 hover:bg-slate-100 transition-all"
          >
            <TrendingUp className="w-4 h-4 text-[#f97316]" /> {t("GST Reports", "GST నివేదికలు")}
            {!isPaidPlan && (
              <span className="absolute -top-2 -right-2 bg-[#f97316] text-white text-[8px] font-semibold px-1.5 py-0.5 rounded-full uppercase tracking-widest border border-white">PRO</span>
            )}
          </button>
          <button 
            onClick={() => setIsPaymentModalOpen(true)}
            className="bg-[#f97316] text-white px-5 py-2.5 rounded-lg text-xs font-semibold uppercase flex items-center gap-2 hover:bg-[#ea580c] transition-all"
          >
            <Plus className="w-4 h-4" /> {t("New Payment", "కొత్త చెల్లింపు")}
          </button>
        </div>
      </div>
                        ...
                        <div className="flex items-center justify-end gap-3">
                          {balance > 0 && (
                            <button 
                              ...
                              className="bg-primary/10 text-primary text-[10px] font-normal px-3 py-1.5 rounded-lg hover:bg-primary hover:text-white active:scale-[0.95] transition-all uppercase tracking-wider"
                            >
                              {t("PAY", "పేమెంట్")}
                            </button>
                          )}
